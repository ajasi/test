...
const availableChocolates = computed(() => {
  return state.value.chocolatesList
    ? state.value.chocolatesList.filter((chocolate) => !chocolate.deleted && chocolate.quantity > 0)
    : [];
});

const unavailableChocolates = computed(() => {
  return state.value.chocolatesList
    ? state.value.chocolatesList.filter(
        (chocolate) => !chocolate.deleted && chocolate.quantity === 0
      )
    : [];
});
...
<template>
  <div>
    <div class="pb-5 bg-white rounded shadow mb-5">
      <div class="header d-flex px-3">
        ...
        <div class="mt-3 ms-4">
          ...
          <p class="m-0">
            {{ state.factoryData.locationId.latitude }} |
            {{ state.factoryData.locationId.longitude }}
          </p>
    ...
    <div class="bg-white shadow rounded p-3 mt-5">
      <h3>Comments</h3>
      <Comments
        v-for="(comment, index) in state.factoryData?.comments"
        :key="index"
        :comment="comment"
      />
    </div>
